...
namespace LibplctagWrapper
{
    public class Controller : IDisposable
    {
        ...
        public string GetStringValue(Tag tag)
        {
            ...
            int length = GetUint8Value(tag, 0);

            //Builds message.
            StringBuilder sb = new StringBuilder();
            for (int j = 0; j < length; j++)
            {
                char text = (char)GetUint8Value(tag, 4 + j);
                sb.Append(text);
            }
            return sb.ToString();
        }

        public void SetStringValue(Tag tag, string str)
        {
            //Find number of characters in string.
            int length = str.Count();

            //Set character length to first byte.
            SetUint8Value(tag, 0, Convert.ToByte(length));

            //write each character to tag.
            for (int j = 0; j < length; j++)
            {
                SetUint8Value(tag, j + 4, Convert.ToByte(str[j]));
                WriteTag(tag, 100);
            }
        }
